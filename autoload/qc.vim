...
export def TextTr()
	...
	var commands = [
		...
		{text: "Fix trailing spaces", key: "t", close: true, cmd: (_) => {
			exe "FixTrailingSpaces"
			exe "normal!" mode()
		}},
...
export def Git()
	...
	main_commands += [
		{text: 'pull', key: "p", close: true, cmd: (_) => {
...
def SourceVim(...args: list<any>)
	if len(args) == 0
		&opfunc = expand('<stack>')->matchstr('<SNR>\d\+_SourceVim')
		feedkeys('g@')
		return
	endif
...
export def LeaderNormal()
	...
	var commands = [
		...
		{text: "Navigate buffers", key: "n", close: true, cmd: (_) => Nav()},
