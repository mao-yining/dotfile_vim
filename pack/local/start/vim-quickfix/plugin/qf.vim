...
augroup qf
	...
	execute($'autocmd QuickFixCmdPost {quickfix_cmd_pattern} qf.COpen()')
	execute($'autocmd QuickFixCmdPost {loc_list_cmd_pattern} qf.LOpen()')
	...
	if exists('*timer_start')
		autocmd QuickFixCmdPost  helpgrep ++nested timer_start(10, (_) => execute("qf.COpen()"))
		autocmd QuickFixCmdPost lhelpgrep ++nested timer_start(10, (_) => execute("qf.LOpen()"))
	else
		autocmd QuickFixCmdPost helpgrep ++nested qf.COpen()
	endif

	# special case for $ vim -q
	autocmd VimEnter * ++nested if count(v:argv, '-q') | qf.COpen() | endif
